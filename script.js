...
// ✅ 同步兩個下拉選單（避免選錯倉）
const qSel = document.getElementById("warehouse"); // 查詢用
const sSel = document.getElementById("spreadsheetWarehouse"); // 開表用

if (qSel && sSel) {
qSel.addEventListener("change", () => {
if (sSel.value !== qSel.value) sSel.value = qSel.value;
});
sSel.addEventListener("change", () => {
if (qSel.value !== sSel.value) qSel.value = sSel.value;
qSel.dispatchEvent(new Event("change")); // 重新載入資料表
});
}

// ✅ 出勤查詢用：當選擇倉別後更新資料表
qSel.addEventListener("change", function () {
const warehouse = this.value.trim();
...
fetch(`${url}?mode=getSheets&ts=${Date.now()}`)
.then(res => {
...
const base = scriptURLs[warehouse];
if (!base) {
alert("找不到對應的倉別網址！");
return;
}

let url = `${base}?sheet=${encodeURIComponent(sheet)}&name=${encodeURIComponent(name)}&from=glitch&ts=${Date.now()}`;
if (phone) url += `&phone=${encodeURIComponent(phone)}`;

// 偵錯輸出，確保真的是 TAO10
console.log("[DEBUG] 倉別=", warehouse, " URL=", url);
alert(`即將查詢：${warehouse}\nURL：${url}`);
...
const warehouse = document.getElementById("spreadsheetWarehouse").value.trim();
